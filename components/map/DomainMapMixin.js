...
import {map_additional_layers} from "~/lib/domain_data"
...
export default {
  ...
  computed: {
    ...
    available_layers() {
      return this.$_.get(this.get_domain_data, "map.layers", [])
    },
  ...
  watch: {
    map_loaded() {
      // console.log("DomainMap loaded", this.layer_status)
      if (!this.layer_status) {
        ...
        const default_active_layers = this.$_.get(this.get_domain_data, "map.default_active_layers", []).filter(l => available_layer_names.includes(l))
        this.set_layer_visibility(default_active_layers)
      }
      this.load_additional_layers(map_additional_layers(this.get_domain_data))
    }
  }
}
