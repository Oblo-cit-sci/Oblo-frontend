...
export default {
  ...
  methods: {
    async guarantee_entry(entry_uuid, entry_access_key = null) {
      // console.log("fetch.guarantee_entry", entry_uuid);
      if (this.$store.getters["entries/has_full_entry"](entry_uuid)) {
        // console.log("fetch.guarantee_entry HAVE IT ALREADY")
        // console.log(this.$store.getters["entries/get_entry"](entry_uuid))
        return this.$store.getters["entries/get_entry"](entry_uuid)
      } else {
        try {
          let entry_response = null
          // console.log("fetch.guarantee_entry FETCHING...")
          if (!entry_access_key) {
          ...
          // console.log("fetch.guarantee_entry RECEIVED...")
          ...
          if (entry_response.status === 200) {
            ...
            // console.log("guarantee_entry...", entry.uuid)
            return entry
          } else {
            // console.log("fetch.guarantee_entry REJECT...")
            return entry_response
          }
        } catch (e) {
          // console.log("fetch.guarantee_entry REJECT2...")
          return e
        }
      }
    },
