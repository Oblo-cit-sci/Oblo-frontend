  ...
  export default {
    ...
    computed: {
      disabled_options() {
        ...
        // basically a set of all the codes get_code_of_template returns for all selected templates
        const include_codes = []

        if (template_filter) {
          console.log(template_filter.value)
          for(let template_slug of template_filter.value) {
            const template_refs = get_code_of_template(this.$store, template_slug)
            console.log("template_refs", template_refs)
            for(let code of template_refs) {
              if(!include_codes.includes(code)) {
                include_codes.push(code)
              }
            }
          }
        }
        console.log(this.options)
        console.log("disabled options", this.options.filter(o => !include_codes.includes(o.value)).map(o => o.value))
        return this.options.filter(o => !include_codes.includes(o.value)).map(o => o.value)
      }
    }
  }
</script>
