...
export default {
  ...
  methods: {
    ...
    get_text_of_mvalues(mvalue, aspect) {
      /**
      ...
      if ([MULTISELECT, SELECT].includes(aspect_type)) {
        ...
        return aspect.items.filter(i => unpack(mvalue).includes(i.value)).map(v => v.text).join(", ")
        ...
        return ""
        // debugger
        // return this.get_text_of_mvalues(unpack(mvalue), option_aspect)
      } else {
      }
