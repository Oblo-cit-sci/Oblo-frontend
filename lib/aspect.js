...
import {ENTRY_ROLES, HEAD, TREEMULTISELECT, VIEW_COMPONENT} from "~/lib/consts";
...
export function value_text(aspect, raw_value) {
  ...
  switch (aspect.type) {
    case SELECT:
      const selected_option = aspect.items.find(i => i.value === raw_value)
      return ld.get(selected_option, "text", raw_value)
    case MULTISELECT:
    case TREEMULTISELECT: {
      debugger
