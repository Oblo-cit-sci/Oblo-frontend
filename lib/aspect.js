...
import {ACTORS, ENTRY_ROLES, ENTRYLINK, HEAD, TREEMULTISELECT, VIEW_COMPONENT} from "~/lib/consts";
import EntryRolesAspect from "~/components/aspects/EntryRolesAspect";
import {ENTRIES_GET_EDIT, ENTRIES_VALUE} from "~/store/entries";
...
export function unpack(value) {
  if (typeof value === "object" && Object.keys(value).length === 1 && value.hasOwnProperty("value")) {
    return value.value
  } else {
    return value
  }
...
export function aspect_loc_str2arr(aspect_loc_str, list_index = null) {
...
export function get_aspect_vue_component(aspect, mode = EDIT) {
  ...
  } else if(mode === EDIT && aspect.attr.edit_component) {
    switch (aspect.attr.edit_component) {
      case "tag_options":
        return TagOptions
...
export function check_condition_value(condition_value, condition) {
  ...
  switch (compare) {
    ...
    case "contains":
      ...
      return ld.includes(condition.value, condition_value)
    default:
...
export function first_edit_loc(store) {
  return [[EDIT, store.getters[ENTRIES_GET_EDIT]().uuid]]
}
  ...
  if (typeof aspects === "object") {
    if (aspects.constructor === Array) {
      ...
      for (let aspect of Object.entries(aspects)) {
        ...
        values[aspect[0]] = unpack(aspect[1].value)
      }
    }
  }
  return values
