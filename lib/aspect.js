...
export function pack_propper_value(value = null) {
  /**
   * packs it normally but packs arrays properly.
   * Use this for Multiselect packing...
   */
  if (Array.isArray(value)) {
    return {value: value.map(v => pack_propper_value(v))}
  } else {
    return {value: value}
  }
...
export function isEqual_default_value(mvalue, aspect) {
  const default_value = aspect_default_value(aspect)
  if (aspect.type === OPTIONS) {
    if (mvalue.option) {
      const option_aspect = aspect.options.filter(o => o.name === mvalue.option)[0]
      return ld.isEqual(mvalue.value, aspect_default_value(option_aspect))
    } else {
      return ld.isEqual(mvalue, default_value)
    }
  } else {
    return ld.isEqual(mvalue, default_value)
  }
...
export function type_default_value(aspectType) {
  return pack_value(aspectTypeDefault(aspectType))
}
...
export function get_list_index(aspect_loc) {
  return aspect_loc.filter(a => a[0] === INDEX).map(al => al[1])
}

export function fix_index(aspect_loc, indices) {
  let next_index = 0
  aspect_loc.forEach(al => {
    if (al[0] === INDEX) {
      if (indices.length >= next_index) {
        al[1] = indices[next_index]
        next_index++
      }
    }
  })
