import {ENTRIES_VALUE} from "./store_consts";
...
export function loc_append(prev, atype, avalue) {
  return ld.concat(prev, [[atype, avalue]])
}
...
export function packed_aspect_calced_value(aspect) {
  return pack_value(aspect_calced_value(aspect))
}
...
export function disabled_by_condition(store, aspect, aspect_loc) {
  if (aspect.attr.hasOwnProperty("condition")) {

    let aspect_location = complete_aspect_loc(
      aspect_loc_uuid(aspect_loc),
      aspect_loc_str2arr(aspect.attr.condition.aspect)) // todo index missing
