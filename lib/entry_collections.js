...
export function entries2vuetify_tree(entries, entry_types, add_outdated) {
  const items = []
  ...
  const rec_item = (uuid) => {
    ...
    const child_items = []
    if (add_outdated) {
      if (entry_types[e.type_slug])
        item.outdated = (e.parent_type_version || 0) !== entry_types[e.type_slug].version
    }
