...
import {ENTRIES_ALL_ENTRIES_ARRAY, ENTRIES_VALUE} from "~/store/entries";
...
export function has_meta_aspect(entry, {meta_aspect_name, conditional_value, compare}) {
  // console.log("has_meta_aspect", entry.title, meta_aspect_name, conditional_value)
  // console.log(entry)
  ...
  if (entry.hasOwnProperty(meta_aspect_name)) {
    if(!conditional_value)
      ...
      if(compare && compare === "unequal") {
        return entry[meta_aspect_name] !== conditional_value
      } else {
        return entry[meta_aspect_name] === conditional_value
      }
    }
...
export async function local_search(store, search_config) {
  ...
  if (search_config.required.domain !== NO_DOMAIN) {
    ...
    for (let entry of entries) {
      ...
      const etype = store.getters[TEMPLATES_TYPE](entry.type_slug)
      if (!etype) {
  ...
  */
  ...
  for (let entry of req_filtered_entries) {
    ...
    if (search_config.include.aspect_search) {
      const etype = store.getters[TEMPLATES_TYPE](entry_value.type_slug)
