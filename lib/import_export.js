import {loc_prepend, loc_remove_last} from "./aspect";
import {ENTRY} from "./consts";
import {set_titleAspect} from "./entry";
...
export function merge_imported_entries(store, entries) {
  ...
  for (let entry of entries) {
    if (entry.entry_refs.parent !== null) {
  ...
  for (let u of uuids) {
    ...
    store.commit("entries/save_entry", e_map[u])
  }
  // fix_check_entrylist_references
  for (let e of new_entries) {
    console.log(e.template_version, store.getters["templates/entry_type"](e.type_slug).version)
  }
  return new_entries
}
...
function recursive_merge(store, entry, entry_dict) {
  ...
  //console.log("rec-merge", entry.title)
  if (store.getters["entries/has_entry"](entry.uuid)) {
    const store_entry = store.getters["entries/get_entry"](entry.uuid)
    const child_uuids = entry.entry_refs.children || {}
    ...
    for (let child_uuid in child_uuids) {
      ...
      if (ld.get(store_entry, "entry_refs.children.child_uuid")) {
        // merge, with message
        ...
        const entry_type = store.getters["templates/entry_type"](child_entry.type_slug)
        new_entries.push(child_entry)
        // new_entries[entry_type.slug] = (new_entries[entry_type.slug] || 0) + 1
        const child_aspect_loc = entry.entry_refs.children[child_uuid]
        ...
        const store_elist_val = ld.cloneDeep(store.getters["entries/value"](store_elist_aspect))
        ...
        if (ld.find(all_type_entries, e => e.title === child_entry.title)) {
          // this was a method in entry.js before

          child_entry.title = entry_type.title
          set_titleAspect(store, entry)
        }

        store.dispatch("entries/save_child_n_ref", {
          uuid: entry.uuid,
          child: child_entry,
          aspect_loc: aspect_loc_destination
        })
        store.dispatch("entries/set_entry_value", {
