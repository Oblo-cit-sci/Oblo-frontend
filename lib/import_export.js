...
import {find_orhpans} from "~/lib/fixes";
import {ENTRIES_GET_RECURSIVE_ENTRIES} from "~/lib/store_consts";
...
export function merge_imported_entries(store, entries) {
  ...
  for(let e of new_entries) {
    console.log(e.parent_type_version, store.getters[ENTRYTYPES_TYPE](e.type_slug).version)
  }
  return new_entries
}
...
export function upload_to_repo(store, axios, entries, url, include_orphans) {
  /**
   * @type {getters.user_key|default.computed.user_key|(function(*): *)|(function(): *)|w.computed.user_key|(function(): *)|*}
   */
  const user_key = store.getters.user_key
  if (!user_key) {
    // return new Error("nana")
    // return new Promise(function(suc, fail) {
    //   throw new Error("nana")
    // })
    return Promise.reject("No user key. Go to the settings and paste the user key given by the LICCI core team")
    // this.error_snackbar()
    // return
  }
  //
  if(include_orphans){
    const orphan_uuids = find_orhpans(store)
    console.log("adding",orphan_uuids.length, "orphans")
    for(let uuid of orphan_uuids) {
      entries.push(store.getters[ENTRIES_GET_ENTRY](uuid))
    }
  }
  let export_data = {entries: entries, user_key: user_key}
  return upload(axios, url, export_data)
}
