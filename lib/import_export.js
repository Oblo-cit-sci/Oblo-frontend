import {
  ...
  ENTRIES_VALUE, ENTRYTYPES_TYPE, ENTRYTYPES_TYPENAME
} from "./store_consts";
...
export function merge_imported_entries(store, entries) {
  ...
  for (let entry of entries) {
    //console.log(entry)
    let etype = store.getters[ENTRYTYPES_TYPE](entry.type_slug)
    //if(etype.)
...
function recursive_straight_insert(store, entry, entry_dict) {
  ...
  if(entry_type) {
    ...
    console.log("skipping entry for unknwon type", entry.type_slug)
  }
  return []
}
