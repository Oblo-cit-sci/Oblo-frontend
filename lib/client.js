import {get_entry_value, get_local_entry} from "./entry";
...
export function resolve_aspect_ref(store, entry, aspect_descr) {
  //console.log("aspect-ref", aspect_descr)
  if (aspect_descr.attr.value) {
    const val_def = aspect_descr.attr.value.toString()
    //console.log("value?", aspect_descr.attr, val_def)
    if (val_def.startsWith("#")) { // a reference attribute
      let value = get_entry_value(entry, val_def)
      //console.log("value", value)
    }
  } else {
    return aspect_descr.attr.value
  }
}
...
export const OWNER = {key: "OWNER", actors_key: "owners"}
...
const ActorEntryRelations = [OWNER, EXPLICIT_ACCESS, COLLABORATOR]
...
export function entry_actor_relation(entry, user) {
  ...
  if (identity(actors.creator))
    ...
    for (let ae_relation of ActorEntryRelations) {
      ...
      if (in_group !== -1)
        if (ae_relation.key === OWNER.key && in_group === 0) {
