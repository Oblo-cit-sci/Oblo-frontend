...
import {CONNECTING, CONNECTION, DB_LOADED, ENTRIES_SAVE_ENTRY, INIT, SET_DOMAIN} from "./store_consts";
...
import {SEARCH_PREPEND_ENTRIES, SEARCH_SET_ENTRIES, SEARCH_SET_SEARCH_COUNT} from "~/store/search";
...
export function search_entries(api, store, config, prepend) {
  ...
  if (domain_entry) {
    api.domain__$domain_name__entries(domain_entry.conditional_value,
      ld.get(config, "page.limit", 100), ld.get(config, "page.offset", 0),
      config)
      .then(({data}) => {
        const count = data.data.count
        const cachable_entries = data.data.entries
        const entry_uuids = []
        for(let e of cachable_entries) {
          if (typeof e === "object") {
            this.$store.commit(ENTRIES_SAVE_ENTRY, e)
          }
        }
