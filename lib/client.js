...
/*
export function fix_entries(store) {
  console.log(store.state.entries.entries)
  console.log(store.state.entries.entries.entries())
  const latest_version = pkg.version
  let re_add = []
  for (let e of store.state.entries.entries.entries()) {
    if (true) { // e[1].version !== latest_version
      e[1].version = latest_version
      re_add.push(e[1])
      store.commit("entries/delete_entry", e[0])
    }
  }
  for (let e of re_add) {
    store.commit("entries/save_entry", e)
