import {get_entry_value, get_local_entry, is_aspect_ref_string} from "./entry";

const ld = require("lodash");

import entry_types from "../lib/types";
import codes from "../lib/codes"
import {PRIVATE, PRIVATE_LOCAL, PUBLIC, VISITOR} from "./consts";
import {aspect_loc_str2arr} from "./aspect";
...
//import pre_data from "../assets/data/test"
  ...
  for (let aspect of aspects) {
    ...
    try {
      if (aspect.attr.hasOwnProperty("condition")) {
        ...
        if (relations.hasOwnProperty(aspect.attr.condition.aspect)) {
          relations[aspect.attr.condition.aspect].push(aspect.name)
        } else {
...
export function resolve_aspect_ref(store, entry, aspect_descr) {
  //console.log("client.resolve_aspect_ref", aspect_descr)
  if (aspect_descr.attr.ref_value) {
    const aspect_loc_str = aspect_descr.attr.ref_value
    let location_array = aspect_loc_str2arr(aspect_loc_str)
    // TODO the reason why we hack "entries/get_entry_value" with an entry parameter...
    let value = store.getters["entries/get_entry_value"]({uuid:entry.uuid, aspect_loc: location_array, entry: entry})
    console.log("VALLLLYUYUEE", value)
    //console.log("value?", aspect_descr.attr, val_def)
    return value
  } else {
    return undefined
  }
...
export function get_entries_of_type(store, type) {
  return ld.filter(Array.from(store.state.entries.entries.values()), (d) => {
