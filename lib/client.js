...
import entries from "../lib/data_backups/entries";
import {
  EDIT,
  ENTRY,
  ENTRY_INDEX,
...
export async function search_entries(axios, keyword) {
  // I would like to use the same state of entries and
  // depending on the page, shohw pubics or privates. Right
  // now I am using global entries, and getting data from backup json
  console.log("api get for entries");
   // return await axios.get(`/entries`,{
    //  params: {
    //    keyword: keyword
    //  }
    // }).then((res) => {
    //  store.commit("entries/save_entry", res.result)
    //  })
     // .catch((e) => {
