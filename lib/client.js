...
import {CONNECTING, DB_LOADED, DOMAIN, SET_DOMAIN, SET_TEMPLATES_CODES_FOR_DOMAIN} from "~/store";
...
export async function initialize(api, store, route) {
  ...
  if (auth_token.access_token) {
    api.actor__validate_token(auth_token).then(res => {
  ...
  // console.log(data)
  const domains_basic_data = data.data
  console.log("base", domains_basic_data)
  // .then(({data}) => {
  ...
  // // todo simplify by all templates and codes...
  const calls = []
  for (let domain of domains_basic_data) {
    console.log(domain.name)
    calls.push(api.domain__$domain_name__basic_entries(domain.name))
  }
  const domains_data_responses = await Promise.all(calls);
  const domains_full_data = ld.zip(domains_basic_data, domains_data_responses)
  //
  // // console.log(domains_data)
  for (let full_domain of domains_full_data) {
    const basic_data = full_domain[0]
    const templates = full_domain[1]
    console.log(basic_data)
    console.log(templates)
    // const domains_data = domains_data_response.data
    // store.dispatch(SET_TEMPLATES_CODES_FOR_DOMAIN, {
    //   domain_name: basic_data.name,
    //   entries: data.data
    // })
  }
