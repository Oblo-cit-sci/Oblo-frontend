...
export async function search_entries(axios, store, config) {
    ...
    if(domain_entry) {
        fetch_domain_entries(axios, domain_entry.conditional_value,
          ld.get(config, "page.limit", 20), ld.get(config, "page.offset", 0), [REGULAR],
            false).then(res => {
            // console.log(res)
...
export async function initialize(axios, store, route) {
    store.commit(CONNECTING, true)
    //console.log("connecting")
    axios.get("/domain").then((resp) => {
    ...
    get_domain_templates_n_codes(NO_DOMAIN, store, axios).then(({data}) => {
  ...
  if(route.name === "domain-domain") {
    ...
    get_domain_templates_n_codes(route.params.domain, store, axios).then(({data}) => {
...
export async function get_domain_templates_n_codes(domain_name, store, axios) {
    return axios.get(`/domain/${domain_name}/basic_entries`, {
        // params: {
...
export async function fetch_domain_entries(axios, domain_name, limit, offset) {
    return axios.get("/domain/" + domain_name + "/entries", {
        params: {
            limit,
            offset
        }
