...
export function tree_code_has_tag_rule(tree_entry) {
  return !_.isEmpty(code_entry_tags_rule(tree_entry))
}

export function build_tag_select_tree(tree_entry) {
  const tag_levels = _.get(code_entry_tags_rule(tree_entry), "from_tree.levels", [])
...
export function get_code_of_template(store, template_slug, language) {
  return _.get(store.getters["templates/entry_type"](template_slug, language), "entry_refs")
    .filter(c_ref => c_ref.ref_type === "tag")
    .map(c_ref => c_ref.dest_slug)
}
