import {
  ENTRIES_ALL_ENTRIES_ARRAY, ENTRIES_GET_ENTRY,
  ENTRIES_SAVE_ENTRY,
  ENTRIES_SET_ENTRY_VALUE,
  ENTRIES_VALUE, ENTRYTYPES_TYPE,
} from "./store_consts";
import {ENTRIES_SET_ENTRY_STATUS, ENTRYTYPES_HAS_TYPE, ENTRYTYPES_TYPES} from "~/lib/store_consts";
...
export function find_orhpans(store) {
  ...
  for (let e of entries) {
    if (!store.getters[ENTRYTYPES_HAS_TYPE](e.type_slug)) {
      ...
      if (store.getters[ENTRYTYPES_TYPE](e.type_slug).content.meta.context === GLOBAL) {
...
export function fix_broken_aspect_value_structure(entry, e_type, store) {
  ...
  for (let [orig_name, new_name] of Object.entries(mapping)) {
    if (outdated.includes(orig_name) && missing.includes(new_name)) {
      ...
      const value = entry.aspects_values[orig_name]
...
export function kick_redundent_values(entry, e_type) {
  ...
  for (let aspect_name of outdated) {
    delete new_entry.aspects_values[aspect_name]
  }
  return new_entry
}
