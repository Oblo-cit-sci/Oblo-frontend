...
import {ENTRIES_SAVE_ENTRY} from "./store_consts";
...
// todo DEPR replace with store.release_mode
export function get_release_mode(store, context) {
  if (context) {
    return context.env.release_mode
  } else {
    return store.app.context.env.release_mode
  }
}
...
export function load_eovalue_db(store) {
  ...
  const licci = [] //require("./eovalue_dbs/licci")
  ...
  licci.forEach(entry => {
    ...
    store.dispatch(ENTRIES_SAVE_ENTRY, entry.uuid)
  })
...
export function route_change_query(route, new_query_params) {
  return {
    path: route.path,
    query: Object.assign(ld.cloneDeep(route.query), new_query_params)
  }
}
