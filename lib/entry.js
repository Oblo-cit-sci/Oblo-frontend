import {ASPECT, DRAFT, EDIT, PRIVATE_LOCAL, STR, TITLE_ASPECT, VIEW} from "./consts"
...
export function MAspectComponent(aspect, ignore_page = false, mode = EDIT) {
  ...
  if (pageAspect && !ignore_page) {
    return AspectPageButton
  } else if (!pageAspect || ignore_page) {
    if (aspect.type === "str") {
...
export function aspect_default_value(aspect) {
  ...
  switch (aspect.type) {
    ...
    case "tree":
      return null
    case "composite":
      return ld.map(aspect.components, (c) => aspect_wrapped_default_value(c, {name: c.name}))
    case "options":
      return null
    case "select":
      return null
...
export function aspect_wrapped_default_value(aspect, merge = {}) {
  return Object.assign({value: aspect_default_value(aspect)}, merge)
}
...
export function create_entry(store, type_slug, init = {}) {
  ...
  const entry =  {
    ...
    license: entry_type.content.meta.has_license === false ? "None" : user_data.defaultLicense,
    privacy: entry_type.content.meta.has_privacy === false ? PRIVATE_LOCAL : user_data.defaultPrivacy,
...
export function delete_entry(store, uuid) {
  store.commit("entries/delete_entry", uuid)
}
