...
export function full_title(store, entry) {
  return store.getters["templates/type_name"](entry.template.slug) + (entry.title ? ": " + entry.title : "")
}
...
export function temp_direct_select(entries_mod, loc) {
  ...
  if (pre_setter.includes(parts[0])) {
    ...
    if (pre_set === "$edit") {
      ...
      select = entries_mod.entries.get(parts[1]).values
      loc = cut_from(loc,2)
    }
  }
