...
import {get_release_mode} from "./util";
...
export function create_entry(store, type_slug, init = {}, parent = null) {
  ...
  if (entry_type.content.meta.titleAspect) {
    // is this the best way to check something and have a default of
    // set title aspect.default true
    if (entry_type.content.meta.hasOwnProperty("setTitleAspect")) {
      if (entry_type.content.meta.setTitleAspect) {
...
export function set_titleAspect(store, entry) {
  ...
  let set_aspect = entry_type.content.meta.hasOwnProperty("set_aspect") ? entry_type.content.meta.set_aspect : true
  if (set_aspect) {
    const loc_arr = aspect_loc_str2arr(entry_type.content.meta.titleAspect)
