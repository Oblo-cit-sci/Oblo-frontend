...
export function get_local_entry(store, ref) {
  console.log("local entry", ref)
  if (ref.draft_id !== undefined) {
    return store.state.edrafts.drafts[ref.draft_id]
  } else if (ref.local_id !== undefined) {
...
export function get_ref_entries(store, refs) {
  const ref_ids = ld.map(refs, ref => ref.local_id)
  //console.log("ref_ids", ref_ids)
  let result = Array()
  for(let ow of store.state.entries.own_entries.values()) {
    //console.log("e", ow.local_id)
    //console.log()
    if(ref_ids.indexOf(ow.local_id) > -1) {
      result.push(ow)
    }
  }
  return result
  // todo ups, map and lodash...?!
  //return ld.filter(store.state.entries.own_entries.values(), (entry) => {return ref_ids.indexOf(entry.local_id) > -1})
}
