    ...
    try {
      ...
      if (skip_collect) {
        ...
        select.value.forEach(e_uuid => {
          ...
          if (val[LIST]) {
            // todo replace all values by uuid
            collected = ld.concat(collected, val.value)
            if (A_TYPE === ENTRY_COLLECT_LINK) {
              console.log("WARNING, TODO,", ENTRY_COLLECT_LINK, " not done for lists yet, convert all entries values to their uuid...")
            }
          } else {
            if (A_TYPE === ENTRY_COLLECT_LINK) {
              collected = ld.concat(collected, {value: e_uuid, text: val.text})
            } else {
