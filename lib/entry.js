import {ASPECT, DRAFT, EDIT, INDEX, PRIVATE_LOCAL, STR, TITLE_ASPECT, VIEW} from "./consts"
...
export function aspect_raw_default_value(aspect) {
  ...
  try {
    ...
    console.log("cant get default value of aspect: ", aspect)
  }
}
...
export function set_entry_value(entry, aspect_loc, value) {
  ...
  //console.log("setting value of", entry)
...
/*
  ...
  see aspect.js
 */
export function get_entry_value(entry, aspect_loc) {
  let select = entry.aspects_values
  //console.log("setting value of", entry)

  aspect_loc = aspect_loc_str2arr(aspect_loc)
  //console.log("loc arr", aspect_loc, aspect_loc[1])
...
export function aspect_loc_str(aspect_loc) {
  ...
  return ld.map(aspect_loc, al => al[0][0] + "-" + al[1]).join("_").replace(" ", "_")
}
