...
import {ENTRIES_SAVE_ENTRY, ENTRYTYPES_TYPE, ENTRYTYPES_TYPENAME, UPDATE_DRAFT_NUMBER, USER_DATA} from "./store_consts";
...
export function create_entry(store, type_slug, init = {}, parent = null) {
  ...
  const entry_type = store.getters[ENTRYTYPES_TYPE](type_slug)
  // console.log("type of type_slug",entry_type)
  const draft_no = get_update_draft_no(store, type_slug)

  store.dispatch("user/guarantee_uuid")
  const user_data = store.getters[USER_DATA]
  const title = init.title || entry_type.title + " " + draft_no

  //console.log("ET", entry_type)
  const entry = {
    ...
    license:
      entry_type.rules.license ? entry_type.rules.license :
        (entry_type.rules.privacy === PRIVATE_LOCAL ? "None" : user_data.defaultLicense),
    privacy: entry_type.rules.privacy ? entry_type.rules.privacy : user_data.defaultPrivacy,
    title: title,
    refs: {
      parent: parent,
      children: {}
    ...
    domain: entry_type.domain,
    downloads: false, // todo: should go to local
    creation_datetime: new Date()
  }
  if (entry_type.rules.titleAspect) {
    // is this the best way to check something and have a default of
    // set title aspect.default true
    if (entry_type.rules.hasOwnProperty("setTitleAspect")) {
      if (entry_type.rules.setTitleAspect) {
...
export function set_titleAspect(store, entry) {
  ...
  let set_aspect = entry_type.rules.hasOwnProperty("set_aspect") ? entry_type.rules.set_aspect : true
  if (set_aspect) {
    const loc_arr = aspect_loc_str2arr(entry_type.rules.titleAspect)
    let title_val = select_aspect_loc(null, loc_arr, false, entry.aspects_values)
...
export function get_entry_titleAspect(entry_type) {
  if (entry_type.rules.hasOwnProperty(TITLE_ASPECT)) {
    return entry_type.rules.titleAspect
  } else {
    if (entry_type.aspects[0].type === STR) {
      return "# " + entry_type.aspects[0].name
    } else {
...
export function get_update_draft_no(store, entry_type) {
  // entry_type is type_slug
  const draft_no = store.getters.draft_no(entry_type) + 1
...
export function default_values(entry_type) {
  let values = {}
  let aspects = entry_type.aspects
...
export function has_pages(entry_type) {
  return entry_type.rules.hasOwnProperty("pages")
}
