...
import {ENTRIES_SAVE_ENTRY, UPDATE_DRAFT_NUMBER} from "./store_consts";
...
export function create_entry(store, type_slug, init = {}, parent = null) {
  ...
  const user_data = store.getters["user/user_data"]
...
function get_update_draft_no(store, entry_type) {
  ...
  const has_title_aspect = store.getters.entry_type(entry_type).content.meta.hasOwnProperty("titleAspect")
  let draft_no = null
  if (has_title_aspect) {
    draft_no = store.getters.draft_no(entry_type)
    store.commit(UPDATE_DRAFT_NUMBER, entry_type)
  } else {
    console.log("getting draft number from count")
    draft_no = store.getters["entries/all_entries_of_type"](entry_type).length + 1
    //store.commit(UPDATE_DRAFT_NUMBER, entry_type)
  }
  return draft_no
}
