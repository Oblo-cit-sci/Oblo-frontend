...
import {entry_actor_relation, user_ref} from "./actors";
import {recursive_unpack} from "./util";
import {CREATOR} from "~/lib/actors";
import {META, REGULAR} from "~/lib/consts";
import {UPDATE_DRAFT_NUMBER, USER_GET_USER_DATA} from "~/store";
...
export function create_entry(store, template_slug, init = {}, parent = null) {
  // todo different owner in case of visitor
  // console.log("entry of template_slug", type_slug)
  const template = store.getters[ENTRYTYPES_TYPE](template_slug)
  // console.log("type of template_slug",template)
  const draft_no = get_update_draft_no(store, template_slug)
  ...
  const entry = {
    ...
    privacy: template.rules.privacy ? template.rules.privacy : user_data.default_privacy,
    title: title,
    refs: {
      parent: parent,
      children: {}
...
export function set_titleAspect(store, entry) {
  const template = store.getters[ENTRYTYPES_TYPE](entry.template.slug)
...
export function full_name(store, entry) {
  return store.getters[ENTRYTYPES_TYPENAME](entry.template.slug) + ": " + entry.title
}

export function get_proper_mode(entry, store) {
  // when entry is private local, always > edit
  // whenlocationAspectlocationAspect owner, draft > edit
  // otherwise > view
  let relation = entry_actor_relation(entry, store.getters.user)
  if (relation === CREATOR.actors_key) {
    return EDIT
  } else {
    return VIEW
  }
