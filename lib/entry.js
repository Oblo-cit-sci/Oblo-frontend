...
export function create_entry(store, template_slug, init = {}, parent = null) {
  ...
  const template = store.getters[TEMPLATES_TYPE](template_slug)
...
export function set_titleAspect(store, entry) {
  const template = store.getters[TEMPLATES_TYPE](entry.template.slug)
...
export function full_name(store, entry) {
  return store.getters[TEMPLATES_TYPENAME](entry.template.slug) + ": " + entry.title
}
