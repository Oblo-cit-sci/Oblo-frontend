...
export const LOCATION_PRECISION_REGION = "region"
...
export function create_location_error(coords, max_km = 50, min_km = 5) {
  /**
   * experiment:
   * @type {number}
   */
  const ang = 2 * Math.random() * Math.PI
  const rad = (min_km + Math.random() * (max_km - min_km)) / 111
...
export function entry_location2geojson_arr(entry, include_props = [], no_title_str) {

  const location = entry.location
  const base_properties = {
    uuid: entry.uuid,
    template: entry.template.slug,
    title: entry.title === "" ? no_title_str : entry.title,
    tags: entry.tags,
    location
  }
  ...
  for (let i in location) {
    ...
    // TODO THIS SHOULD NOT BE PACKED IN THE FIRST PLACE!
    let loc_i = null
    if(location[i].value) {
      loc_i = unpack(location[i])
      logger.warn("location is packed...")
    } else {
      loc_i = location[i]
    }
    features.push({
      type: "Feature",
      geometry: {
        type: "Point", "coordinates": coords2array(loc_i.coordinates)
      },
