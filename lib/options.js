...
export function object_list2options(values, text_from, value_from, clean = false) {
  return ld.map(values, (c) => {
    return Object.assign(clean ? {} : ld.cloneDeep(c), {text: c["text"] || c[text_from], value: c["value"] || c[value_from]})
  })
}
...
export function get_codes_as_options(store, code_name) {
  code_name = code_name.substring(1)
  const code_parts = code_name
  let code_entry = store.getters["get_code"](code_name)
  ...
  if (code_entry.hasOwnProperty("type_slug")) {
    if (code_entry.type_slug === "valuemap") {
